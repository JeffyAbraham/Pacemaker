...
public class PacemakerConsoleService {
  ...
  public void follow(@Param(name = "email") String email) {
	  Optional<User> user = Optional.fromNullable(loggedInUser);
	    if (user.isPresent()) {
	      console.println(paceApi.follow(user.get().id, email));
	    }
  }

  @Command(description = "List Friends: List all message of logged in user")
  public void listMessages() {
	  Optional<User> user = Optional.fromNullable(loggedInUser);
	    if (user.isPresent()) {
	      console.println(paceApi.getmessages(user.get().id).toString());
	    }
  }
  ...
  public void friendActivityReport(@Param(name = "email") String email) {
	  User u=paceApi.getUserByEmail(email);
	  Optional<User> user=Optional.fromNullable(loggedInUser);
	  System.out.println(u.firstname);
	  if(user.isPresent())
	  {		
		  console.renderActivities(paceApi.listActivities(u.id, "type"));
	  }
  ...
  public void unfollowFriend(@Param(name = "email") String email) {
	  Optional<User> user = Optional.fromNullable(loggedInUser);
	    if (user.isPresent()) {
	      console.println(paceApi.unfollow(user.get().id, email));
	    }
  }
  ...
  public void messageFriend(@Param(name = "email") String email,
	  ...
	  Optional<User> user = Optional.fromNullable(loggedInUser);
	    if (user.isPresent()) {
	    	System.out.print("inside mf");
	      paceApi.messageFriend(user.get().id, email, message);
	    }
  }

  @Command(description = "List Friends: List all friends for the logged in user")
  public void listFriends() {
	  Optional<User> user = Optional.fromNullable(loggedInUser);
	    if (user.isPresent()) {
	    	Collection<User> s=paceApi.getfriends(user.get().id);
	    
	      console.println(paceApi.getfriends(user.get().id).toString());
	    }
  }
